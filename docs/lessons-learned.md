...
## 2026-03-31: FK Constraint Fix for Virtual Key Provider Configs

### Problem
Service failed to start with `FOREIGN KEY constraint failed` when creating virtual key provider configs from config.json.

### Root Cause
Virtual key provider configs had `"rate_limit_id": ""` (empty string) instead of `null` or omitted. GORM attempted to create FK relationship with non-existent rate limit ID `""`.

### Solution
Add normalization logic before creating/updating provider configs:

```go
// In transports/bifrost-http/lib/config.go
// Normalize empty strings to nil for optional FK fields
...
### Lesson
Always normalize empty strings to nil for optional foreign key fields. This handles configs generated by UI forms or scripts that don't distinguish between "not set" and "empty string".
