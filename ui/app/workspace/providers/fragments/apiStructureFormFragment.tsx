...
export function ApiStructureFormFragment({ provider }: Props) {
  const hasUpdateProviderAccess = useRbac(
    RbacResource.ModelProvider,
    RbacOperation.Update,
  );
  const dispatch = useAppDispatch();
  const [updateProvider, { isLoading: isUpdatingProvider }] =
  ...
  const form = useForm<FormCustomProviderConfig>({
    ...
    defaultValues: {
      ...
      allowed_requests: {
        list_models:
          provider.custom_provider_config?.allowed_requests?.list_models ??
          true,
        text_completion:
          provider.custom_provider_config?.allowed_requests?.text_completion ??
          true,
        text_completion_stream:
          provider.custom_provider_config?.allowed_requests
            ?.text_completion_stream ?? true,
        chat_completion:
          provider.custom_provider_config?.allowed_requests?.chat_completion ??
          true,
        chat_completion_stream:
          provider.custom_provider_config?.allowed_requests
            ?.chat_completion_stream ?? true,
        responses:
          provider.custom_provider_config?.allowed_requests?.responses ?? true,
        responses_stream:
          provider.custom_provider_config?.allowed_requests?.responses_stream ??
          true,
        embedding:
          provider.custom_provider_config?.allowed_requests?.embedding ?? true,
        speech:
          provider.custom_provider_config?.allowed_requests?.speech ?? true,
        speech_stream:
          provider.custom_provider_config?.allowed_requests?.speech_stream ??
          true,
        transcription:
          provider.custom_provider_config?.allowed_requests?.transcription ??
          true,
        transcription_stream:
          provider.custom_provider_config?.allowed_requests
            ?.transcription_stream ?? true,
        image_generation:
          provider.custom_provider_config?.allowed_requests?.image_generation ??
          true,
        image_generation_stream:
          provider.custom_provider_config?.allowed_requests
            ?.image_generation_stream ?? true,
        image_edit:
          provider.custom_provider_config?.allowed_requests?.image_edit ?? true,
        image_edit_stream:
          provider.custom_provider_config?.allowed_requests
            ?.image_edit_stream ?? true,
        image_variation:
          provider.custom_provider_config?.allowed_requests?.image_variation ??
          true,
        count_tokens:
          provider.custom_provider_config?.allowed_requests?.count_tokens ??
          true,
        ocr: provider.custom_provider_config?.allowed_requests?.ocr ?? true,
        ocr_stream:
          provider.custom_provider_config?.allowed_requests?.ocr_stream ?? true,
      },
      request_path_overrides:
        provider.custom_provider_config?.request_path_overrides ?? undefined,
    },
  });
    ...
    updateProvider(
      buildProviderUpdatePayload(provider, {
        custom_provider_config: {
          base_provider_type:
            data.base_provider_type as unknown as BaseProvider,
          is_key_less: data.is_key_less ?? false,
          allowed_requests: data.allowed_requests,
          request_path_overrides: cleanPathOverrides(
            data.request_path_overrides,
          ),
        },
      }),
    )
