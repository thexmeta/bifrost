...
func (gs *LocalGovernanceStore) loadFromConfigMemory(ctx context.Context, config *configstore.GovernanceConfig) error {
	...
	budgetMap := make(map[string]*configstoreTables.TableBudget, len(budgets))
	for i := range budgets {
		budgetMap[budgets[i].ID] = &budgets[i]
	}

	rateLimitMap := make(map[string]*configstoreTables.TableRateLimit, len(rateLimits))
	for i := range rateLimits {
		rateLimitMap[rateLimits[i].ID] = &rateLimits[i]
	}

	teamMap := make(map[string]*configstoreTables.TableTeam, len(teams))
	for i := range teams {
		teamMap[teams[i].ID] = &teams[i]
	}

	customerMap := make(map[string]*configstoreTables.TableCustomer, len(customers))
	...
	for i := range modelConfigs {
		...
		// Populate budget
		if mc.BudgetID != nil {
			if budget, ok := budgetMap[*mc.BudgetID]; ok {
				mc.Budget = budget
			}
		}
		...
		if mc.RateLimitID != nil {
			if rateLimit, ok := rateLimitMap[*mc.RateLimitID]; ok {
	...
	for i := range providers {
		...
		if provider.BudgetID != nil {
			if budget, ok := budgetMap[*provider.BudgetID]; ok {
		...
		if provider.RateLimitID != nil {
			if rateLimit, ok := rateLimitMap[*provider.RateLimitID]; ok {
	...
	for i := range virtualKeys {
		...
		if vk.TeamID != nil {
			if team, ok := teamMap[*vk.TeamID]; ok {
				vk.Team = team
			}
		}

		if vk.CustomerID != nil {
			if customer, ok := customerMap[*vk.CustomerID]; ok {
				vk.Customer = customer
			}
		}

		if vk.RateLimitID != nil {
			if rateLimit, ok := rateLimitMap[*vk.RateLimitID]; ok {
		...
		if vk.ProviderConfigs != nil {
			for j := range vk.ProviderConfigs {
				...
				if pc.RateLimitID != nil {
					if rateLimit, ok := rateLimitMap[*pc.RateLimitID]; ok {
